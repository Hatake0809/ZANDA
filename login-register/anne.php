...
<body>
    ...
    <div class="container" style="height:700px ; margin-left:15% ; animation: slide 2s ease forwards;  animation-delay: calc(.2s * var(--i)); ">
            <h2 class="text-uppercase" style="color:white ; font-size:40px; margin-left:35%">
                Anne, Bebek, Oyuncak
            </h2>
            <hr>
        <div class="row">
            <div class="col-md-3">                				
                ...
                <div class="list-group">
                    ...
                    $query = "SELECT DISTINCT(anne_brand) FROM anne WHERE anne_status = '1' ORDER BY anne_id DESC";
                    ...
                    <div class="list-group-item checkbox">
                        <label><input type="checkbox" class="common_selector brand" value="<?php echo $row['anne_brand']; ?>"  > <?php echo $row['anne_brand']; ?></label>
                    </div>
                    <?php
                    }
				...
				<div class="list-group">
                    ...
                    $query = "
                    SELECT DISTINCT(anne_size) FROM anne WHERE anne_status = '1' ORDER BY anne_size DESC
                    ";
                    $statement = $connect->prepare($query);
                    $statement->execute();
                    $result = $statement->fetchAll();
                    ...
                    ?>
                    <div class="list-group-item checkbox">
                        <label><input type="checkbox" class="common_selector size" value="<?php echo $row['anne_size']; ?>" > <?php echo $row['anne_size']; ?></label>
                    </div>
                    <?php    
                    }
				...
				<div class="list-group">
					...
					<?php
                    $query = "
                    SELECT DISTINCT(anne_color) FROM anne WHERE anne_status = '1' ORDER BY anne_color DESC
                    ";
                    $statement = $connect->prepare($query);
                    $statement->execute();
                    $result = $statement->fetchAll();
                    ...
                    ?>
                    <div class="list-group-item checkbox">
                        <label><input type="checkbox" class="common_selector color" value="<?php echo $row['anne_color']; ?>"  > <?php echo $row['anne_color']; ?></label>
                    </div>
                    <?php
                    }
                    ?>	
...
$(document).ready(function(){
    ...
    function filter_data()
    {
        ...
        $.ajax({
            url:"filitreleme_anne.php",
            method:"POST",
            data:{action:action, minimum_price:minimum_price, maximum_price:maximum_price, brand:brand, size:size, color:color},
